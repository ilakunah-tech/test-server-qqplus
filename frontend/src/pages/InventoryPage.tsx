...
export const InventoryPage = () => {
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      if (e.key === 'Escape') { setArrivalCoffee(null); }
    };
    window.addEventListener('keydown', onKey);
    return () => window.removeEventListener('keydown', onKey);
  }, []);

  const handleCoffeeSubmit = (data: CoffeeFormData) => {
    if (editingCoffee) {
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="space-y-4">
        ...
        {/* ── Единая форма создания/редактирования кофе ── */}
        <AddCoffeeDialog
          open={showCoffeeForm || !!editingCoffee}
          onClose={() => { setShowCoffeeForm(false); setEditingCoffee(null); }}
          onSubmit={handleCoffeeSubmit}
          coffee={editingCoffee}
          isPending={createCoffeeMutation.isPending || updateCoffeeMutation.isPending}
        />

        {/* ── Приход зерна ── */}
        {arrivalCoffee && (
          <>
            <div className="fixed inset-0 z-40 bg-black/30" aria-hidden onClick={() => setArrivalCoffee(null)} />
            <div className="fixed left-1/2 top-1/2 z-50 w-full max-w-sm -translate-x-1/2 -translate-y-1/2 rounded-card border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 p-6 shadow-xl">
              ...
              <form
                onSubmit={(e) => {
                  ...
                  if (kg > 0) addStockMutation.mutate({ id: arrivalCoffee.id, weightKg: kg });
                }}
                className="space-y-4"
              >
                <div>
                  <Label htmlFor="weight_kg">Вес, кг *</Label>
                  <Input id="weight_kg" name="weight_kg" type="number" step="0.01" min="0.01" required placeholder="0.00" className="mt-1" />
                </div>
                <p className="text-xs text-gray-500 dark:text-gray-400">
                  На складе: {formatWeight(Number(arrivalCoffee.stock_weight_kg) || 0)}
                </p>
                <div className="flex gap-2 justify-end pt-2">
                  <Button type="button" variant="outline" onClick={() => setArrivalCoffee(null)}>Отмена</Button>
                  <Button type="submit" disabled={addStockMutation.isPending}>Добавить</Button>
                </div>
              </form>
            </div>
          </>
                ...
                <tr key={coffee.id} className="hover:bg-gray-50 dark:hover:bg-gray-700/50">
                  ...
                  <td className="px-4 py-3">
                    <div className="flex gap-2">
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => setArrivalCoffee(coffee)} title="Приход">
                        <PackagePlus className="w-4 h-4" />
                      </Button>
                      <Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => { setEditingCoffee(coffee); setShowCoffeeForm(true); }} title="Edit">
                        <Pencil className="w-4 h-4" />
                      </Button>
                      <Button
                        variant="ghost" size="icon"
                        className="h-8 w-8 text-red-600 hover:text-red-700 hover:bg-red-50"
                        onClick={() => { if (window.confirm('Удалить это зелёное зерно?')) deleteCoffeeMutation.mutate(coffee.id); }}
